...
- 知识库治理：导入时强制标注来源类型、权威等级和审核状态；支持在 Governance 面板修正文档内容并重新索引，正式检索默认只使用 published 文档，并保留版本历史、审计日志和处理进度监控；删除、重处理和内容版本更新会同步清理旧 MinIO 资源与旧向量。
