...
- 文档导入：支持文件上传、批量上传、纯文本导入、URL 导入，以及 JPEG/PNG 图片作为独立知识源导入。
- 自动去重：同一知识库内按文档名称和 SHA-256 文件 hash 跳过重复文件。
- 文档解析：支持 Markdown、TXT、HTML、PDF、DOCX、PPT、PPTX、XLSX、JPEG 和 PNG；Markdown/HTML 输出 heading、paragraph、table、image、code 等结构化 chunks，并可提取内嵌 data URI 图片；PDF/Office 默认优先使用 MinerU content_list 结构化输出，保留 text/table/image/equation chunk、page、bbox、标题路径和图片/表格说明。
- 图片与资源保存：独立上传的图片、文档中的图片和解析出的图片资源会保存到 MinIO，并能在控制台 chunk 视图中渲染。
- 多模态 RAG：导入时使用轻量 CLIP 模型为图片资源建立视觉向量索引；检索时用同一个 CLIP 文本向量召回相关图片，并与文本 dense/sparse 召回融合。
...
- 知识库治理：导入时强制标注用户自定义的来源类型、权威等级和审核状态；支持在 Governance 面板修正文档内容并重新索引，正式检索默认只使用 published 文档，并保留版本历史、审计日志和处理进度监控；删除、重处理和内容版本更新会同步清理旧 MinIO 资源与旧向量。
...
- 文档处理：Markdown/HTML 结构化解析、MinerU content_list 结构化解析、pypdf 轻量回退、python-docx 回退、自定义结构化 chunker。
- 模型服务：Infinity embedding/rerank HTTP 服务，本地 CLIP 小模型用于图片-文本跨模态召回。
...
- `QDRANT_TIMEOUT_SECONDS`、`QDRANT_UPSERT_BATCH_SIZE`：控制导入最后写入 Qdrant 的操作超时和分批大小；如果文档在 indexing / Writing vectors to Qdrant 阶段超时，优先增大 timeout 或减小 batch size。
- `EMBEDDING_TIMEOUT_SECONDS`、`EMBEDDING_BATCH_SIZE`、`EMBEDDING_MAX_INPUT_CHARS`：控制导入阶段调用 embedding 服务的超时、分批大小和单 chunk 向量化输入上限；CPU 版 Infinity 跑 `BAAI/bge-m3` 时建议从较小 batch 开始。处理大型 XLSX 或表格文档时，如果出现 embedding `ReadTimeout`，优先减小 batch size 或适当增大 timeout。
- `PDF_PARSER` 与 `MINERU_*`：控制 PDF 解析方式，默认使用 MinerU。
- `MULTIMODAL_RAG_ENABLED` 与 `CLIP_*`：控制图片视觉向量索引和 CLIP 检索，默认使用 `openai/clip-vit-base-patch32`；`CLIP_PRELOAD_ON_STARTUP=true` 时 API、MCP Server 和 Worker 启动阶段会先下载并加载 CLIP。
...
`mineru[core]` 已在项目依赖中，当前 MinerU CLI 支持 PDF、图片、DOCX、PPT、PPTX、XLSX 输入。首次安装可能较慢，因为 MinerU 和文档解析相关依赖较大；如果你的部署环境需要 MinerU 的完整推理后端，可在兼容的 Python/torch 环境中额外安装 `mineru[all]`。如果你只想快速验证轻量功能，可以在 `.env` 中把 `PDF_PARSER` 调整为 `pypdf`，但完整 PDF/Office 表格、公式、图片和版面解析建议使用默认 MinerU 配置。MinerU 的 `content_list.json` 会被转换成 typed chunks：相邻 text chunk 会按知识库 chunk size/overlap 合并，table/image/equation 会保持独立 chunk，并写入 `chunk_type`、`page_num`、`bbox`、`title_path`、`table_caption`、`image_caption`、`image_path`、`latex` 等元数据。
...
知识源分级、发布规则、版本历史和审计日志见 [docs/governance.md](docs/governance.md)。
...
AGENT_LLM_API_KEY=
AGENT_LLM_MODEL=qwen2.5vl:7b
AGENT_LLM_TIMEOUT_SECONDS=60
AGENT_LLM_TEMPERATURE=0.2
AGENT_LLM_MULTIMODAL_ENABLED=true
AGENT_LLM_IMAGE_DETAIL=auto
AGENT_MAX_CONTEXT_CHARS=12000
AGENT_MAX_IMAGE_ASSETS=3
AGENT_MAX_IMAGE_BYTES=2000000
```

如果召回 chunk 中包含由 BYOB 解析出的图片或独立上传的 JPEG/PNG，检索结果会带 `assets` manifest。控制台 QA Agent 会通过 MCP 拉取这些图片，并按 OpenAI-compatible `image_url` 格式交给支持多模态的模型。每张图片前都会附带 source id、asset id 和 Markdown URL，方便视觉模型把图片内容和 `[S1]`、`[S2]` 这类来源引用对应起来；最终 Markdown 回答也会保留或补充相关图片/文件链接。`AGENT_LLM_MULTIMODAL_ENABLED=false` 时 Agent 只把图片作为文本 asset manifest 提供，不会把图片 bytes 发给 LLM。部分 OpenAI-compatible 服务会限制温度参数；例如 Moonshot/Kimi 某些模型要求 `AGENT_LLM_TEMPERATURE=1`。

多模态 RAG 默认启用。文档处理时，独立 JPEG/PNG 上传和 MinerU 解析出的图片都会保存到 MinIO，同时写入同知识库配套的 Qdrant 视觉集合 `<collection>_visual`。普通检索、MCP 检索和 QA Agent 都会默认开启 `enable_visual_search`，因此类似“图里展示的趋势是什么”这类查询可以先通过 CLIP 找到相关图片，再把对应 chunk 与图片 asset 一起交给 Agent。默认 `CLIP_PRELOAD_ON_STARTUP=true`，首次启动 API、MCP Server 或 Worker 时会先下载并加载 CLIP 模型；如果希望更快启动并接受首次视觉检索时再加载，可以改为 `false`。
...
- Knowledge base management with document, chunk, governance, and indexing status tracking.
- File upload, batch upload, text import, URL import, and standalone JPEG/PNG image import.
- Duplicate skipping by document name and SHA-256 file hash within the same knowledge base.
- Markdown, TXT, HTML, PDF, DOCX, PPT, PPTX, XLSX, JPEG, and PNG parsing. Markdown/HTML parsing emits heading, paragraph, table, image, and code chunks and extracts embedded data URI images. PDF/Office parsing uses MinerU content_list output by default for layout-aware text/table/image/equation chunks, page/bbox metadata, title paths, tables, formulas, and OCR-friendly output.
- Standalone uploaded images, document images, and extracted image assets are stored in MinIO, with rendered chunk previews in the console.
- Multimodal RAG with a lightweight CLIP model that indexes extracted images and fuses text-to-image visual hits with dense/sparse text retrieval.
...
- Governance workspace for updating source labels, editing retrievable document content, re-indexing, monitoring version/audit history, and cleaning old MinIO resources plus old vectors during delete/reprocess/content updates.
...
- Document processing: Markdown/HTML structured parsing, MinerU content_list structured parsing, pypdf lightweight fallback, python-docx fallback, custom structured chunkers.
- Model services: Infinity embedding/rerank HTTP services plus a local small CLIP model for cross-modal image retrieval.
...
- `QDRANT_TIMEOUT_SECONDS` and `QDRANT_UPSERT_BATCH_SIZE`: control Qdrant operation timeout and ingestion vector upsert batch size. If documents time out during indexing / Writing vectors to Qdrant, increase the timeout or reduce the batch size.
- `EMBEDDING_TIMEOUT_SECONDS`, `EMBEDDING_BATCH_SIZE`, and `EMBEDDING_MAX_INPUT_CHARS`: control embedding request timeout, ingestion batch size, and the per-chunk text bound sent to the embedding service. Start with smaller batches when running `BAAI/bge-m3` on the CPU Infinity image. For large XLSX or table-heavy documents, reduce the batch size or increase the timeout if embedding `ReadTimeout` appears.
- `PDF_PARSER` and `MINERU_*`: control PDF parsing behavior.
- `MULTIMODAL_RAG_ENABLED` and `CLIP_*`: control visual image indexing and CLIP retrieval. The default model is `openai/clip-vit-base-patch32`; with `CLIP_PRELOAD_ON_STARTUP=true`, the API, MCP Server, and Worker download and load CLIP during startup.
...
`mineru[core]` is part of the project dependencies, and the current MinerU CLI supports PDF, image, DOCX, PPT, PPTX, and XLSX inputs. The first install can take a while because document parsing dependencies are large; deployments that need MinerU's full inference backends can install `mineru[all]` in a compatible Python/torch environment. For lightweight local checks you can set `PDF_PARSER=pypdf`, but full PDF/Office layout, table, image, and formula extraction should use the default MinerU setup. MinerU `content_list.json` is converted into typed chunks: adjacent text chunks are merged by knowledge-base chunk size/overlap, while table/image/equation chunks stay atomic and store metadata such as `chunk_type`, `page_num`, `bbox`, `title_path`, `table_caption`, `image_caption`, `image_path`, and `latex`.
...
When retrieved chunks reference extracted or standalone uploaded images, retrieval results include an `assets` manifest. The console QA Agent loads those images through MCP and sends them as OpenAI-compatible `image_url` inputs to multimodal-capable models. Each image block is preceded by source id, asset id, and Markdown URL text so the vision model can map the image back to `[S1]`, `[S2]`, and the rendered source asset link. The final Markdown answer also preserves or appends relevant image/file links. Set `AGENT_LLM_MULTIMODAL_ENABLED=false` to keep images as text-only asset manifests without sending image bytes to the LLM. Some OpenAI-compatible providers restrict temperature values; for example, some Moonshot/Kimi models require `AGENT_LLM_TEMPERATURE=1`.

Multimodal RAG is enabled by default. During ingestion, standalone JPEG/PNG uploads and extracted images are stored in MinIO and indexed into a companion Qdrant visual collection named `<collection>_visual`. Standard retrieval, MCP retrieval, and the QA Agent enable `enable_visual_search` by default, so text queries can retrieve relevant images through CLIP and pass the matched chunk plus image assets into the Agent. By default `CLIP_PRELOAD_ON_STARTUP=true`, so the API, MCP Server, or Worker downloads and loads CLIP at startup; set it to `false` for faster startup with lazy first-use loading.
