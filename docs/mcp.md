...
默认情况下，检索只返回 `review_status = published` 的文档。`document` 中会包含 `governance_source_type`、`authority_level`、`review_status` 和 `version`。`governance_source_type` 和 `authority_level` 的具体含义由团队维护；BYOB 默认在冲突时把数字更小的 `authority_level` 视为更高优先级。
