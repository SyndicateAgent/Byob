...
标准混合检索工具，复用 BYOB 的 dense + sparse + rerank 检索链路。
...
返回结果中的 `results[].content` 是原文 chunk，`document` 包含来源文档，`stats` 包含 embedding、Qdrant recall、sparse search、rerank、内容回表等耗时。
...
3. 如果命中结果来自同一文档且上下文不足，调用 `get_document_chunks` 扩展上下文。
4. 基于 `results[].content` 生成答案，并引用 `document.name`、`chunk_id` 或页码 metadata。
