...
4. 知识库治理：导入时强制标注来源类型、权威等级和审核状态，检索默认只使用 published 文档，并保留版本历史与审计日志。
...
| PDF 解析 | MinerU 优先，pypdf 兜底 | PDF 版面、表格、公式、OCR 友好解析 |
...
- PDF 文档优先使用 MinerU 解析 Markdown/content-list 结构化输出，提升复杂版面、表格、公式和扫描类 PDF 的抽取准确度；MinerU 不可用时可配置回退到 pypdf。
...
documents
  ...
  source_type, source_url, governance_source_type, authority_level,
  review_status, current_version, status, error_message, metadata,
  chunk_count, created_at, updated_at

document_versions
  id, document_id, kb_id, version_number, source metadata,
  governance fields, change_summary, created_by, created_at

document_audit_logs
  id, document_id, kb_id, actor, action, summary,
